...
export const ProductReviewForm: React.FC<ProductReviewFormProps> = ({
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-200 shadow-md overflow-hidden max-w-2xl mx-auto my-4 animate-in fade-in duration-200">
      {/* Header */}
              ...
              >
                ...
                {deliveredProducts.map((item) => (
                  <option key={item.productId} value={item.productId}>
                    {item.productName} (Reseller Price: ৳{item.resellerPrice})
                  </option>
                ))}
              </select>
            )}
              ...
              {[1, 2, 3, 4, 5].map((starVal) => {
                ...
                return (
                  <button
                    ...
                    className="p-1 rounded-lg hover:bg-amber-50 focus:outline-none transition-all cursor-pointer group active:scale-95"
            ...
            {imagePreview ? (
              <div className="relative w-32 h-32 rounded-xl border-2 border-dashed border-slate-300 bg-slate-100 overflow-hidden group">
                ...
                <button
                  ...
                  className="absolute top-1.5 right-1.5 bg-slate-900/80 text-white p-1 rounded-lg hover:bg-rose-600 transition-colors shadow-md"
                  title="Remove image"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              <label className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-slate-300 hover:border-blue-500 bg-slate-50 hover:bg-blue-50/50 rounded-xl cursor-pointer transition-all">
                <Upload className="w-6 h-6 text-slate-400 mb-1" />
                <span className="text-xs font-bold text-slate-700">Click to upload photo</span>
                <span className="text-[10px] text-slate-400 mt-0.5">JPEG, PNG or WEBP up to 3MB</span>
                <input
                  type="file"
                  accept="image/*"
