...
import { 
  ...
  X, ArrowLeft, ArrowRight, ShieldCheck, DollarSign, WifiOff
} from 'lucide-react';
...
export const OrderCustomizationForm: React.FC<OrderCustomizationFormProps> = ({
  ...
  return (
    <div className="fixed sm:relative inset-0 sm:inset-auto z-50 sm:z-auto w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-3xl bg-white sm:rounded-2xl border-0 sm:border border-slate-200 shadow-2xl flex flex-col overflow-hidden animate-in fade-in sm:zoom-in-95 duration-200">
      ...
      {placedOrderInfo ? (
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 flex flex-col justify-center items-center text-center space-y-6">
          ...
          {/* Breakdown Card */}
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl w-full max-w-md mx-auto text-left text-xs space-y-2">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2 w-full max-w-md">
            <button
              ...
              className="w-full sm:w-auto px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition-colors cursor-pointer min-h-[44px] flex items-center justify-center"
            >
            ...
            {onCancel && (
              <button
                onClick={onCancel}
                className="w-full sm:w-auto px-6 py-3 bg-slate-900 hover:bg-slate-800 text-white font-extrabold text-xs rounded-xl shadow-md transition-colors cursor-pointer min-h-[44px] flex items-center justify-center"
              >
                Go to Orders List
              </button>
            )}
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-5">
              ...
              {loadingProducts ? (
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full h-12 sm:h-10 px-3.5 bg-slate-50 border border-slate-300 rounded-xl text-base sm:text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 appearance-none cursor-pointer pr-10"
                  >
                    {products.map((p) => (
            ...
            {/* 2. Color Variant (if applicable) with 32px minimum swatches */}
                    ...
                    .map((variant) => {
                      ...
                      return (
                        <button
                          ...
                          className={`p-3 min-h-[56px] rounded-xl border-2 text-left flex items-center gap-3 transition-all cursor-pointer ${
                            isOut
                              ? 'bg-slate-100 border-slate-200 opacity-50 cursor-not-allowed'
                              : isSelected
                              ? 'bg-indigo-50 border-indigo-600 ring-2 ring-indigo-500/20 shadow-xs font-bold'
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-12 h-12 bg-slate-100 hover:bg-slate-200 text-slate-800 rounded-xl font-extrabold text-xl flex items-center justify-center transition-colors cursor-pointer shrink-0"
                >
                  -
                </button>
                ...
                <input
                  ...
                  className={`w-28 h-12 text-center py-2 bg-slate-50 border rounded-xl text-base font-mono font-extrabold text-slate-900 focus:outline-none ${
                    isStockExceeded
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                className="w-full min-h-[100px] p-3.5 bg-slate-50 border border-slate-300 rounded-xl text-base sm:text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
              />
            </div>

            {/* 5. Delivery Preference - Full-Width Tappable Cards */}
            <div className="space-y-2">
              <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {DELIVERY_OPTIONS.map((opt) => {
                  ...
                  return (
                    <label
                      key={opt.id}
                      className={`p-4 min-h-[64px] rounded-xl border-2 flex items-center justify-between gap-3 cursor-pointer transition-all ${
                        isSelected
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-[10px] text-slate-500 font-medium">{opt.duration}</span>
                        </div>
                      </div>
                      <span className="text-xs font-mono font-extrabold text-indigo-700 shrink-0">
                        +৳{opt.cost}
                      </span>
                    </label>
                  );
            ...
            {/* 6. Gift Wrap & Message Card - Large Tappable Area */}
            <div className="space-y-3 p-4 bg-pink-50/80 border border-pink-200 rounded-2xl">
              <label className="flex items-center gap-3 cursor-pointer min-h-[44px]">
                <input
                  type="checkbox"
                  checked={giftWrap}
                  onChange={(e) => setGiftWrap(e.target.checked)}
                  className="w-6 h-6 rounded text-pink-600 focus:ring-pink-500 cursor-pointer accent-pink-600 shrink-0"
                />
              ...
              {giftWrap && (
                <div className="space-y-1.5 pt-2 animate-in fade-in duration-150 border-t border-pink-200">
                  <div className="flex items-center justify-between">
                  ...
                  <textarea
                    ...
                    className="w-full p-3 bg-white border border-pink-300 rounded-xl text-base sm:text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-pink-500/30 min-h-[80px]"
                  />
                </div>
              )}
            </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-11 pl-10 pr-3.5 bg-slate-50 border border-slate-300 rounded-xl text-base sm:text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                      required
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full h-11 pl-10 pr-3.5 bg-slate-50 border border-slate-300 rounded-xl text-base sm:text-xs font-mono font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600"
                      required
                    />
                  </div>
                </div>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    className="w-full pl-10 pr-3.5 py-3 bg-slate-50 border border-slate-300 rounded-xl text-base sm:text-xs font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 min-h-[90px]"
                    required
                  />
                </div>
              </div>
            ...
            <div className="p-4 bg-slate-900 text-white rounded-2xl space-y-2.5">
              ...
              <div className="pt-2 border-t border-slate-800 flex justify-between items-center">
                <span className="text-xs font-bold text-indigo-400 uppercase tracking-wider">Total Amount</span>
          ...
          <div className="sticky bottom-0 z-20 bg-slate-50 px-4 sm:px-6 py-4 border-t border-slate-200 flex items-center justify-end gap-3 shrink-0">
            ...
            <button
              type="submit"
              disabled={submitting || isStockExceeded || !isOnline}
              className="px-6 py-3 sm:py-2.5 bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 text-white font-extrabold text-sm sm:text-xs rounded-xl transition-all shadow-md shadow-emerald-600/30 flex items-center justify-center gap-2 cursor-pointer active:scale-98 min-h-[44px] sm:min-h-[38px]"
            >
              {submitting ? (
                <>
