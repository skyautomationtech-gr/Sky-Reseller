...
import { X, ShoppingBag, Search, Check, AlertCircle, Loader2, Sparkles, AlertTriangle } from 'lucide-react';
...
export const CreateOrderModal: React.FC<CreateOrderModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-900/70 backdrop-blur-xs overflow-y-auto">
      <div className="relative w-full max-w-3xl max-h-[92vh] flex flex-col bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden my-auto animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 bg-slate-900 text-white shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-md shadow-blue-600/30 shrink-0">
              <ShoppingBag className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-sm sm:text-base font-bold">Create New Reseller Order</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto flex-1">
                    ...
                    <div key={item.id} className="p-3.5 flex items-center justify-between gap-4 bg-slate-50/30">
                      <div className="flex items-center gap-3">
                        {mainImage ? (
                          ...
                          <div className="w-12 h-12 bg-slate-100 rounded-xl flex items-center justify-center text-slate-400 text-[10px] font-bold">
                            NO IMG
                          </div>
                        )}
                        <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-slate-50 p-3.5 rounded-xl border border-slate-200">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-1.5 text-xs bg-slate-100 border border-slate-200 rounded-lg text-slate-500 font-mono cursor-not-allowed"
                />
              ...
              <div>
                <label className="block text-[10px] font-bold text-slate-500 uppercase tracking-wider mb-1">
                  order Created name Auto
                </label>
                <input
                  type="text"
                  disabled
                  value={user.fullName}
                  className="w-full px-3 py-1.5 text-xs bg-slate-100 border border-slate-200 rounded-lg text-slate-500 font-semibold cursor-not-allowed"
                />
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            <button
              type="submit"
              disabled={submitting || (isCartMode ? (!cartItems || cartItems.length === 0) : (!selectedProduct || availableStock < 1))}
