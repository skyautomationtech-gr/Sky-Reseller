...
import { Bell, LogOut, ShieldCheck, Store } from 'lucide-react';
    ...
    <header className="bg-white border-b border-slate-200 h-16 px-6 flex items-center justify-between sticky top-0 z-20 shadow-xs">
      ...
      <div className="flex items-center gap-4">
        <div className="hidden sm:flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-1.5 rounded-full text-xs text-slate-600">
          ...
          <span className="font-semibold text-slate-900">{user.shopName}</span>
        ...
        <button
          onClick={onLogout}
          className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white text-xs font-medium py-2 px-3.5 rounded-xl transition-colors shadow-sm"
        >
